  ...
  (parameterize ([current-orig-stx form])
    (syntax-parse form
      ...
      [(define-syntaxes (var:id ...) . rest)
       ;; We trust macros generated by the contract library
  ...
  (parameterize ([current-orig-stx form])
    (syntax-parse form
      ...
      [(define-syntaxes (p/c-for-id) (_ _ _ (_ ctc-id) (_ id) . _))
       ...
       (match-define (tc-result1: ty) (check-contract-app #'ctc-id #'id))
       (register-type #'p/c-for-id ty)
       ...
       (list (make-def-binding #'p/c-for-id ty))]
